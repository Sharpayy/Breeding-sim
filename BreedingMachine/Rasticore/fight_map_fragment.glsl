...
void main()
{
    ...
    for (int i = 0; i < MAX_UNITS; i++)
    {
        if (units[i].z != 0.0)
        {
            ...
            fog = fog + -0.5 + pow(distance((units[i].xy+vec2(512.0))/MapDimensions.xy, uv) * 1.6, sqrt(units[i].z)); //distance((units[i].xy+vec2(512.0))/MapDimensions.xy, uv) * pow(units[i].z, 0.2);
        }
    }
    ...
    * float(t_uv.y > bd.y && t_uv.y < 1.0 - bd.y);
    ...
    //col = col - (col * min(0.7, fog));
    col = mix(col, vec3(0.0), min(0.8, fog / float(MAX_UNITS)));
